 ...
 * Displays Cash Flow statement in years-as-columns format (2023-2052)
 */
...
async function fetchCashFlow(versionId: string): Promise<CFLineItem[]> {
...
export function CashFlowTable({ versionId }: CashFlowTableProps): JSX.Element {
  const { data: lineItems = [], isLoading } = useQuery({
    queryKey: ['cf-statement', versionId],
    queryFn: () => fetchCashFlow(versionId),
  });
  ...
  const handleExportCsv = (): void => {
    exportToCsv(lineItems, `cash-flow-${versionId}.csv`);
  };

  const handleExportExcel = (): void => {
    exportToExcel(lineItems, `cash-flow-${versionId}.xlsx`);
  };
